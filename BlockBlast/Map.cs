...
namespace console_blockBlast_AI
{
    public class Map
    {
        ...
        //private static int FULLCOST = 50;
        //private static int []UNSIMULARITY = {1,3,9*3,27,81}; //kinda works if set to values of power of 3 
        //TODO PUNISH ISLANDS
        public int GetScore()
        {
            float score = 0;
            for (int y = 0; y < S.HEIGHT; y++)
                for (int x = 0; x < S.WIDTH; x++)
                {
                    ...
                    score += costs.unsimularity[unsimularity];

                    if (IsFull(x, y)) score += costs.fullCost;
                }
