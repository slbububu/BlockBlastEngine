...
namespace console_blockBlast_AI
{
    public static class GpuKernels
    {
            ...
            int seedOffset)
        {
            ...
            GpuGame game = new GpuGame(seed, localCosts);
    ...
    public class Evolution
    {
        ...
        public void EvolveRepeatedly(int howManyTimes, int sampleSize = 10, int seedOffset = 0)
        {
            ...
            Console.WriteLine($"Testing with {S.BLOCKCOUNT} block count (standeard is 3)");
            Console.WriteLine("################################################");
            Console.WriteLine();
        ...
        public void Evolve(int sampleSize = 10, int seedOffset = 0)
        {
            ...
            // This version is usually more compatible with modern versions
            //using var d_results = accelerator.Allocate2D<int, Stride2D.DenseX>(index2D.X, index2D.Y); //doesnt work
            //using var d_results = accelerator.Allocate2D<int>(index2D);                               //doesnt work
            using var d_results = accelerator.Allocate2DDenseX<int>(new Index2D(index2D.X,index2D.Y));  //does work
            ...
            unsafe
            {
                ...
                Console.WriteLine($"Multiplied: {baseCost.arrayOfValues[bestAvgIndex % S.COST_COUNT]} by {mult}" +
