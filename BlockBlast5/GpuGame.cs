...
namespace console_blockBlast_AI
{
    ...
    public unsafe struct GpuGame //class -> struct      + no "new" keyword
    {
        ...
        public GpuGame(uint seed, GpuCosts costsX)
        {
            this.rngState = seed;
            costs = costsX;
            gameOver = false;
        ...
        public void DoTheBestMove()
        {
            ...
            for (int i = 0; i < S.BLOCKCOUNT; i++)
            {
                if (IsAPiceInSlot(i))
                {
                    for (int x = 0; x < S.WIDTH; x++)
                    {
                        for (int y = 0; y < S.HEIGHT; y++)
                        {
                            if (this.CanIPlace(i, x, y))
                            {
                                GpuGame sim1 = this;
                                sim1.PlacePice(i, x, y, false);

                                int score = sim1.EvaluateDepth3();
                                if (score < bestScore) bestScore = score;
                            }
                        }
                    }
            ...
            gameOver = (bestScore == int.MaxValue);
            if (!gameOver) PlacePice(bestPiceIndex, bestToX, bestToY);
        }
        private int EvaluateDepth2()
        {
            int bestScore = int.MaxValue;
            bool allSlotsEmpty = true;
            ...
            return allSlotsEmpty ? GetScore() : bestScore;
        }
        private int EvaluateDepth3()
        {
            int bestScore = int.MaxValue;
            bool allSlotsEmpty = true;
